...
\\{speedbar-key-map}"
  ...
  (save-excursion
    ...
    (if (and (boundp 'temp-buffer-show-function)
	     (boundp 'temp-buffer-show-function))
	(progn (make-local-variable 'temp-buffer-show-function)
	       (setq temp-buffer-show-function temp-buffer-show-function)))
...
redirected into a window on the attached frame."
  ...
  (cond ((not speedbar-xemacsp)
	 (run-hooks 'temp-buffer-show-function))
	((fboundp 'run-hook-with-args)
	 (run-hook-with-args 'temp-buffer-show-function buffer))
	((and (boundp 'temp-buffer-show-function)
	      (listp temp-buffer-show-function))
