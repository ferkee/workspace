...
(fset 'sql-init
   [?s ?e ?t ?  ?t ?a ?b ?  ?o ?f ?f return
       ?s ?e ?t ?  ?l ?i ?n ?e ?s ?i ?z ?e ?  ?2 ?0 ?0 ?0 ?; return 
       ?s ?e ?t ?  ?p ?a ?g ?e ?s ?i ?z ?e ?  ?6 ?0 return 
       ?s ?e ?t ?  ?e ?c ?h ?o ?  ?o ?n return])
...
;	 ))
  ...
  '(lambda ()
     (if (not hide-ifdef-define-alist)
	 (setq hide-ifdef-define-alist
	       '((list3 1 JWF1 JWF2 JWF12 DEBUGIT2 )
					;		    (list2 TWO THREE)
		 )))
     (hide-ifdef-use-define-alist 'list3) ; use list2 by default
     (message "Hiding Hook")
     ))
...
(setq hide-ifdef-mode-hook
	 ...
	 (if (not hide-ifdef-define-alist)
	     (setq hide-ifdef-define-alist
		  '((list1 ONE TWO)
		    (list2 TWO THREE)
		    (list3 JWF1 "1" JWF2 JWF12 DEBUGIT2)
		    )))
	 (hide-ifdef-use-define-alist 'list3) ; use list2 by default
	 ))
...
(setq printer-name "172.18.72.241")
